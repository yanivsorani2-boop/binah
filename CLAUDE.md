...
3. **הסרה** (לאחר אישור בלבד) — מחיקת 25 המאמרים, הוספת 301 ב-netlify.toml, עדכון sitemap.xml, feed.xml, archive.html, עמודי קטגוריה, .protected-checksums.txt.
4. **אימות** — `bash scripts/invariants.sh` (כל 10 הבדיקות חייבות לעבור) + בדיקת הפניות מדגמית.
